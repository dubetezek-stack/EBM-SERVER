...
function renderExplorer() {
  ...
  return '<div class="explorer-layout">' +
    ...
    '<div class="toolbar">' +
      ...
      (isMasterPlus ? '<button class="toolbar-btn" id="btn-upload">' + Icons.upload + ' <span>Upload</span></button>' : '') +
    '</div>' +
...
function renderDrives(drives) {
  ...
  for (var i = 0; i < drives.length; i++) {
    ...
    html += '<div class="drive-card' + offlineClass + '" data-drive-id="' + d.id + '">' +
      ...
      '<div class="drive-info">' +
        ...
        '<div class="drive-space">' + spaceText + '</div>' +
      '</div>' +
    '</div>';
  }
  html += '</div></div>';
...
function renderFileList(files, driveId, subpath) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    html += '<tr class="file-row" data-name="' + escapeHtml(f.name) + '" data-drive-id="' + driveId + '" data-subpath="' + escapeHtml(filePath) + '" data-is-dir="' + (f.isDirectory ? 'true' : 'false') + '">' +
      ...
      (isAdmin ? '<td><button class="btn-icon btn-delete-file" data-name="' + escapeHtml(f.name) + '" data-drive-id="' + driveId + '" data-subpath="' + escapeHtml(filePath) + '" data-is-dir="' + (f.isDirectory ? 'true' : 'false') + '" title="Apagar">' + Icons.trash + '</button></td>' : '') +
    '</tr>';
  }
